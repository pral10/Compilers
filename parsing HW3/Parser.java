...
public class Parser {
     ...
     *
     * <pre>
     *   classDeclaration ::= CLASS IDENTIFIER [ EXTENDS qualifiedIdentifier ] [ IMPLMENTS qualifiedIdentifier { COMMA qualifiedIdentifier } ]classBody
     * </pre>
     *
    ...
    /**
     * Parses a interface declation and returns an AST for it.
     *
     * <pre>
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        // interfaceBody() return a list of members in body, member type JMember
     ...
     *
     * <pre>
     *   interfaceMemberDecl ::= ( VOID | type ) IDENTIFIER formalParameters [ THROWS qualifiedIdentifier { COMMA qualifiedIdentifier }] SEMI
     *                          | type variableDeclarators SEMI
     * </pre>
     *
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (have(VOID)) {
            ...
            // interfaceMember does not have body
            JBlock body = null;
            mustBe(SEMI);
            ...
            if (seeIdentLParen()) {
                ...
                JBlock body = null;
                mustBe(SEMI);
                ArrayList<String> updatedMods = mods;
                updatedMods.add("abstract");
                interfaceMemberDecl = new JMethodDeclaration(line, updatedMods, name, type, params, exceptions, body);
            } else {
                // otherwise, a field.
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            return block();
        } else if (have(BREAK)) {                       // parse BREAK statemet
            mustBe(SEMI);
            return new JBreakStatement(line);
        } else if (have(CONTINUE)) {                    // parse BREAK statemet - same as BREAK
            mustBe((SEMI));
            return new JContinueStatement(line);
        } else if (have(DO)) {                          // parse DO-WHILE
            // statment
            JStatement statements = statement();
            // WHILE
            mustBe(WHILE);
            // parExpression, while condition
            JExpression test = parExpression();
            // SEMI
            mustBe(SEMI);
            // 2 parameters statements, test condition
            return new JDoStatement(line, statements, test);
        } else if (have(FOR)) {
            mustBe(LPAREN);
            // empty array forInit type JStatement
            ArrayList<JStatement> forInit = new ArrayList<JStatement>();
            // [ forInit ] is optional. Seeing SEMI means forInit is skipped
            if (!see(SEMI)) {
                // checking whether forInit is looking at a local variable declaration or not
                if (seeLocalVariableDeclaration()) {
                    // a list containing a single JVariableDeclaration object encapsulating the variable declarators
                    forInit.add(new JVariableDeclaration(line, variableDeclarators(type())));
                } else {
                    // a list containing statement expressions
                    forInit.add(statementExpression());
                    // While match COMMA, scan it and add statement expression to forInit array
                    while (have(COMMA)) {
                        forInit.add(statementExpression());
                    }
                }
            ...
            mustBe(SEMI);
            // [ expression ] is optional - seeing SEMI means expression is skipped
            JExpression expression = null;
            if (!see(SEMI)) {
                expression = expression();
            }
            mustBe(SEMI);
            // empty array forUpdate type JStatement
            ArrayList<JStatement> forUpdate = new ArrayList<JStatement>();
            // [ forUpdate ] is optional. Seeing RPAREN means forUpdate is skipped
            if (!see(RPAREN)) {
                forUpdate.add(statementExpression());
                while (have(COMMA)) {
                    forUpdate.add(statementExpression());
                }
                // otherwise, no forUpdate
            } else {
                forUpdate = null;
            }
            mustBe(RPAREN);
            JStatement statements = statement();
            // looking constructor at j--/src/jminusminus/JForStatement.java file to know exactly how many arguments and type of each arguments JForStatement need.
        ...
        } else if (have(SWITCH)) {                                  // parse switch
            // parExpression, switch condition
            JExpression test = parExpression();
            mustBe(LCURLY);
            // { switchBlockStatementGroup } : exists >= 0
            ArrayList<SwitchStatementGroup> switchBlockStatementGroup = new ArrayList<SwitchStatementGroup>();
            while (!see(RCURLY) && !see(EOF)) {
                switchBlockStatementGroup.add(switchBlockStatementGroup());
            }
            mustBe(RCURLY);
            return new JSwitchStatement(line, test, switchBlockStatementGroup);
        } else if (have(THROW)) {                                   // parse THROW
            // THROW expression SEMI
            JExpression expression = expression();
            mustBe(SEMI);
            return new JThrowStatement(line, expression);
        } else if (have(TRY)) {                                     // parse TRY
            //  TRY block { CATCH LPAREN formalParameter RPAREN blcok } [ FINALLY block ]
            // tryBlock
            ...
            // { CATCH LPAREN formalParameter RPAREN blcok } -- Kleene closure
            while (have(CATCH)) {
                mustBe(LPAREN);
            ...
            return new JTryStatement(line, tryBlock, formalParameters, catchBlocks, finallyBlock);
        } else if (have(WHILE)) {
            JExpression test = parExpression();
    ...
    // return type: SwitchStatementGroup
    private SwitchStatementGroup switchBlockStatementGroup() {
        // empty array switchLabels type JExpression, look at SwitchStatementGroup class to know data type of switchLabels
        ArrayList<JExpression> switchLabels = new ArrayList<JExpression>();
        // parse a blockStatement
        // block include blockStatement and break
        ArrayList<JStatement> block = new ArrayList<JStatement>();

        // parse one or more occurences of switchLabel
        while (see(CASE) || see(DEFAULT)) {
            switchLabels.add(switchLabel());
        }
        // parse a blockStatement
        block.add(blockStatement());
        // if see BREAK: parse BERAK, add Break to block and parse SEMI
        if (have(BREAK)) {
            block.add(new JBreakStatement(scanner.token().line()));
            mustBe(SEMI);
        }
    ...
    private JExpression switchLabel() {
        ...
        if (have(CASE)) {                                       // case 5:      -     switchLabel '5'
            switchLabel = expression();
            mustBe(COLON);
            return switchLabel;
        } else {                                                // default:     -      no switchLabel
            mustBe(DEFAULT);
            mustBe(COLON);
            return switchLabel;
        }
    ...
    private Type basicType() {
        if (have(BOOLEAN)) {
            ...
            return Type.CHAR;
        } else if (have(DOUBLE)) {              // parse basicType DOUBLE
            return Type.DOUBLE;
        } else if (have(INT)) {
            return Type.INT;
        } else if (have(LONG)) {                // parse basicType LONG
            return Type.LONG;
        } else {
    ...
    private JStatement statementExpression() {
        ...
        if (expr instanceof JAssignment
                ...
                // insert pre-decrement expression
                || expr instanceof JPreDecrementOp
                // insert post-increment expression
    ...
    /**
     * Parses an conditional expression and returns an AST for it.
     *
     * <pre>
     *   conditionalExpression ::= conditionalOrExpression [ QUESTION expression COLON conditionalEpression ]
     * </pre>
     *
     * @return an AST for an assignment expression.
     */
    ...
    private JExpression conditionalExpression() {
        ...
        // [QUESTION expression COLON conditionalExpression] - optional
        if (have(QUESTION)) {
            // expression
            JExpression firstRHS = assignmentExpression();
            mustBe(COLON);
            JExpression secondRHS = conditionalExpression();
            // 3 arguments: conditionalOrExpression, expression - firstRHS, conditionalExpression - secondRHS
            return new JConditionalExpression(line, lhs, firstRHS, secondRHS);
        } else {
            // only conditionalOrExpression
            return lhs;
        }
    }
     ...
     *
     * <pre>
     *   conditionalAndExpression ::= equalcibityExpression { LAND equalityExpression }
     * </pre>
     *
     * @return an AST for a conditional-and expression.
     */
    ...
    private JExpression literal() {
        ...
        } else if (have(DOUBLE_LITERAL)) {                  // parse double literal
        ...
        } else if (have(LONG_LITERAL)) {                    // parse long literal
    ...
    private boolean seeBasicType() {
        // add DOUBLE and LONG to basicSType
    ...
    private boolean seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            // add DOUBLE and LONG to referenceType
