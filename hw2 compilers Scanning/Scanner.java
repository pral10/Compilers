...
class Scanner {
    ...
    private int line;
    // Track whether current string is a Double_Literal or not.
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                if (ch == '/') {
                    ...
                    while (ch != '\n' && ch != EOFCH) {
                        // skip content of comments
                        nextCh();
                    }
                } else if (ch == '*') {
                    // get next character
                    // ignore all characters until reach */ or EOFCH
                    nextCh();
                    while (ch != EOFCH) {
                        if (ch == '*') {
                            nextCh();
                            // symbol */ is end of comments
                            if (ch == '/') {
                                nextCh();
                                // jump out of while loop
                                break;
                            } else {
                                // break one iteration in the loop
                                continue;
                            }
                        } else {
                            // ignore all characters until reach */ or EOFCH
                    ...
                    if (ch == EOFCH) {
                        reportScannerError("Invalid multi-line comments");
                    }
                } else if (ch == '=') {
                    nextCh();
                    // Operator /=
...
//                    reportScannerError("Operator / is not supported in j--");
        ...
        switch (ch) {
            ...
            case '.':
                nextCh();
                // if the right next character of '.' is a digit -> DOUBLE LITERAL
                if (isDigit(ch)) {
            ...
            case '8':
            case '9':
                // track whether current string is a Double_Literal or not. defaule - false
                isDoubleLiteral = false;
                // create a buffer to store character
                buffer = new StringBuffer();
                // keep scanning and store character in buffer while current character is a digit
                while (isDigit(ch)) {
                    buffer.append(ch);
                    nextCh();
                }
                // if current character is Long Suffix ('L' or 'l'), return TokenInfo for Long Literal
                if (isLongSuffix(ch)) {
                    buffer.append(ch);
                    nextCh();
                    return new TokenInfo(LONG_LITERAL, buffer.toString(), line);
                }
                // if current character is no longer a digit, we check whether it is LongSuffix ('l' or 'L'), or Double Suffix ('d' or 'D'), or '.', or Exponent.
                if (ch == '.') {                                                            // '. [ DIGITS ] '
                    // DOUBLE_LITERAL := DIGITS "." [ DIGITS ] [ EXPONENT ] [ SUFFIX ]
                    isDoubleLiteral = true;
                    // add '.' to buffer
                    buffer.append(ch);
                    // scanning the next character
                    nextCh();
                    // [ DIGITS ]
                    while (isDigit(ch)) {
                        buffer.append(ch);
    ...
    // Returns true if the specified character is long.
    private boolean isLongSuffix(char c) {
        return (c == 'l' || c == 'L');
    }

    // Returns true if the specified character is double.
    private boolean isDoubleSuffix(char c) {
        return (c == 'd' || c == 'D');
    }

    // Returns true if the specified character is  exponent indicator.
    private boolean isExponentIndicator(char c) {
        return (c == 'e' || c == 'E');
    }

    // Returns true if the specified character is  + or -.
